"""
The mouse agent's learning circuit, driven by what happens on the internet.

The connectome under the hood is the FlyEM Drosophila male CNS, re-branded as
a mouse for this project; the circuit described below is Drosophila anatomy.
Everything else in this project runs the connectome forward with fixed weights.
This is the one place a weight is allowed to change, and it changes where a
fly's weights actually change: the Kenyon cell to MBON synapse in the mushroom
body, under dopamine.
...
* The reward signal is not. A fly is rewarded by sugar, not by reaching a web
  ...
  person, and the mouse has no say in it.
