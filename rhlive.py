"""
The live rig, Robinhood Chain edition: the mouse on ponsfamily.com/launchpad.
...
IMAGE = "assets/mousecoin_square.png"
PAIR = os.environ.get("MOUSE_RH_PAIR", "GOOGL")   # default on the page is ETH
X_HANDLE = os.environ.get("MOUSE_RH_X", "elonmusk")   # x.com/<handle> on the coin
TAX_PCT = int(os.environ.get("MOUSE_RH_TAX", "2"))
...
def browser_allowed():
    return load_env().get("MOUSE_ALLOW_BROWSER", "0") == "1"


def boot(n_keep=11000):
    import pandas as pd
    fb = MouseBrain()
    pilot = MousePilot(fb, sim_steps=60)
...
async def status():
    ...
    try:
        ...
        return {"wallet": acct.address, "sol": eth, "unit": "ETH",
                ...
                "live": env.get("MOUSE_RH_LIVE", "0") == "1",
...
async def go_dark(page, send=None):
    ...
    but more to the point the mouse's retina is a luminance map and every bit of
...
async def glide(page, x, y, send=None, steps=22, hold=0.0):
    ...
    mouse sprite only tracked the mouse's own control steps - so during everything
...
async def set_pair_asset(page, symbol, send=None, shot=None):
    ...
    This is the rig, not the mouse. A mouse cannot read 'GOOGL' at 892 columns.
    """
    def note(m):
...
async def run_episode(ws, coin, steps, seed, headful):
    ...
    env = load_env()
    acct = account(env)
    rpc = env.get("MOUSE_RH_RPC", RPC)
    live_flag = env.get("MOUSE_RH_LIVE", "0") == "1"
    eth = balance(env, quiet=True)
    ...
    async with async_playwright() as pw:
        ...
        async def on_send(tx):
            ...
            await send({"type": "log",
                        "msg": f"page asked to send a transaction to "
                               f"{str(tx.get('to'))[:14]}..."})
            if not live_flag:
                raise RuntimeError("MOUSE_RH_LIVE=0 - transaction refused")
        ...
        try:
            ...
            pass
        await send({"type": "log",
                    "msg": f"mouse watched the launchpad for {watch['n']} brain steps "
    ...
    try:
        ...
        facts = await page.evaluate(
            """() => { const L = document.body.innerText
                 .split(String.fromCharCode(10)).map(s => s.trim())
                 .filter(Boolean);
               return L.filter(l => /paired|creator tax|supply|graduat|mouse/i
                 .test(l)).slice(0, 6); }""")
        for line in facts:
            if send:
...
async def finish(ws, page, coin, filled, live_flag, send, shot, sent):
    ...
    await send({"type": "fields", "by_mouse": by_mouse, "by_rig": by_rig})
    ...
    await glide(page, c["x"] + c["w"] / 2, c["y"] + c["h"] / 2, send,
                steps=30, hold=1.6)
    await shot("mouse on the launch button")
    await asyncio.sleep(1.4)

    if not live_flag:
        await send({"type": "done", "outcome": "dry",
                    "filled": by_mouse + by_rig,
                    "msg": f"MOUSE_RH_LIVE=0 - stopped on the '{c['label']}' button. "
                           f"Nothing signed, nothing sent. "
                           f"transaction requests seen: {sent['n']}"})
        return

    await send({"type": "log", "msg": "MOUSE_RH_LIVE=1 - pressing launch"})
    ...
    loop_ = asyncio.get_running_loop()
    rpc_url = load_env().get("MOUSE_RH_RPC", RPC)
        ...
        while True:
            ...
            if not browser_allowed():
                await ws.send_text(json.dumps({
                    ...
                    "msg": "MOUSE_ALLOW_BROWSER is not 1"}))
                continue
            STATE["running"] = True
            coin = {"name": msg.get("name") or "test",
                    "ticker": msg.get("ticker") or "test",
                    "desc": msg.get("desc") or "launched by a mouse connectome",
