"""
The mouse's Robinhood Chain wallet.
...
into .env (gitignored), and never printed. Only the address is shown.
  ...
  py rhwallet.py new       create the mouse's Robinhood Chain wallet
...
def new():
    env = load_env()
    if env.get("MOUSE_RH_SECRET"):
        print("MOUSE_RH_SECRET is already set in .env - refusing to overwrite.")
        print("Delete that line by hand first if you really want a new wallet.")
        return
    Account.enable_unaudited_hdwallet_features()
    acct = Account.create()
    _write_env_key("MOUSE_RH_SECRET", acct.key.hex())
    _write_env_key("MOUSE_RH_RPC", RPC)
    _write_env_key("MOUSE_RH_LIVE", "0")
    print("created the mouse's Robinhood Chain wallet\n")
...
def balance(env=None, quiet=False):
    ...
    if not quiet:
        ...
        print(f"  live      {'ARMED' if env.get('MOUSE_RH_LIVE') == '1' else 'no (MOUSE_RH_LIVE=0)'}")
