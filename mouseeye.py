"""
The mouse agent's eye and its descending-neuron readout.

The connectome under the hood is the FlyEM Drosophila male CNS, re-branded as
a mouse for this project; the anatomy described here is Drosophila anatomy.
Screen pixels enter through the 892 retinotopic hex columns the dataset assigns
to optic-lobe neurons, into L1 and L2 - the lamina cells directly postsynaptic
to photoreceptors R1-R6. The cursor comes back out of the descending neurons a
fly actually walks with.
"""
import numpy as np
...
class MouseEye:
    """Retinotopic sampling of the screen onto the 892 hex columns."""
    ...
    def look(self, img, cx, cy, fov_w=300, fov_h=210, max_hz=180.0):
        ...
        The mouse's gaze follows its own cursor, so the view is egocentric.
        ...
        the mouse moves, and there is no positional signal to learn from. A tight
        window makes what the mouse sees depend on where it is.
        """
        H, W = img.shape
...
class MousePilot:
    """
    Closed loop: the mouse looks at the form, its descending neurons fire, the
    ...
    def step(self, img, cx, cy, gains=None, seed=0, detail=False):
        ...
        # The click is the mouse stopping. MN9 was the obvious choice - proboscis
        ...
        # mouse's actual visual field, not a picture of the screen.
        try:
