"""
An EIP-1193 wallet for the mouse, injected into the page.
...
(() => {
  ...
  const RDNS = "xyz.mousebrain.wallet";
  ...
  async function request(args) {
    ...
    switch (method) {
      ...
      case "personal_sign":
      case "eth_sign":
        return await window.__mouseEthSign(method, params);
      case "eth_signTypedData":
      case "eth_signTypedData_v3":
      case "eth_signTypedData_v4":
        return await window.__mouseEthSignTyped(params);
      case "eth_sendTransaction":
        return await window.__mouseEthSend(params[0]);
      default:
        // everything else is a plain read: let the node answer
        return await window.__mouseEthRpc(method, params);
    }
  }
...
async def attach(page, acct, rpc, chain_id, allow_send=False, on_send=None,
    ...
    await page.expose_function("__mouseEthSign", eth_sign)
    await page.expose_function("__mouseEthSignTyped", eth_sign_typed)
    await page.expose_function("__mouseEthRpc", eth_rpc)
    await page.expose_function("__mouseEthSend", eth_send)
